...
SELECT_CHOICES = [('price', 'Цена (низкая)'), ('-price', 'Цена (высокая)'), ('-name', 'Название А-Я'),
                  ('name', 'Название Я-А')]
...
class OrderSearchForm(forms.Form):
    start_day = forms.DateField(initial=datetime.date.today, widget=forms.SelectDateWidget(years=range(2020, 2030)),
                                label='Дата начала')
    end_day = forms.DateField(initial=datetime.date.today, widget=forms.SelectDateWidget(years=range(2020, 2030)),
                              label='Дата конца')
